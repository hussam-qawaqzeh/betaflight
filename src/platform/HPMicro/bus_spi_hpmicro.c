...
void spiInitDevice(spiDevice_e device)
{
    ...
    // selected SCLK must divide spiClock exactly.
    uint32_t divisor = MAX(2U, (spiClock + 12000000U - 1U) / 12000000U & ~1U);
...
uint16_t spiCalculateDivider(uint32_t freq)
{
    ...
    const uint32_t divisor = (spiClk / freq / 2U) * 2U;

    // HPM SPI divisors are even.  Keep the value non-zero at both ends of
    // the input range so spiSequenceStart() can safely convert it back to Hz.
