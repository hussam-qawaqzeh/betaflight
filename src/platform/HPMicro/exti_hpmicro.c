...
void EXTIConfig(IO_t io, extiCallbackRec_t *cb, int irqPriority, ioConfig_t config, extiTrigger_t trigger)
{
    ...
    // so map the 0..15 encoded base priority onto 1..7 before comparing
    const uint32_t base = NVIC_PRIORITY_BASE(irqPriority);
    const uint32_t plicPriority = ((base >> 1) >= 7) ? 1U : 8U - (base >> 1);
