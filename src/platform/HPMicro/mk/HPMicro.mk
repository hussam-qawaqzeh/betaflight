...
# ---- Architecture flags ----
# RV32IMAFC: I=Integer, M=Multiply, A=Atomic, F=Single-precision FPU, C=Compressed
...
# HPM SDK uses GCC extensions (braced-groups, large enum values,
# object-to-function pointer casts) that -Wpedantic rejects.
# Also relax format warnings since uint32_t is 'long unsigned int' on RISC-V
# vs 'unsigned int' on ARM, causing format specifier mismatches.
# EXTRA_FLAGS is appended at the end of CFLAGS, overriding earlier -Wpedantic.
# 'override' keeps these relaxations even when EXTRA_FLAGS is passed on the
# command line (e.g. 'make EXTRA_FLAGS=-Werror' in CI).
override EXTRA_FLAGS    += -Wno-pedantic -Wno-error=format -Wno-error=incompatible-pointer-types -Wno-error=sign-compare -Wno-error=override-init -Wno-error=overflow -Wno-error=aggressive-loop-optimizations -Wno-error=double-promotion -Wno-error=implicit-function-declaration -Wno-error=return-type -Wno-missing-braces -Wno-error=int-conversion -Wno-error=type-limits -Wno-error=unused-parameter -g
...
ifneq ($(CONFIG),)
# Derive port board name from CONFIG (lowercase)
HPM_PORT_BOARD := $(shell echo $(CONFIG) | tr '[:upper:]' '[:lower:]')
endif
...
SRC_CFLAGS_serial_4way.c = -Wno-error -Wno-implicit-function-declaration
SRC_CFLAGS_gyro_sync.c = -Wno-error
SRC_CFLAGS_bus_spi.c = -Wno-error=unused-parameter
