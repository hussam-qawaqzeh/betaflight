...
static uint32_t hpmPlicPriorityFromNvic(uint8_t nvicPriority)
{
    // Betaflight encodes 16 priority ranks with lower values more urgent;
    // HPM6300/HPM6700 PLIC priorities range from 1..7 in the opposite order.
    ...
    if (gptmr_check_status(UART_DIR_SWITCH_GPTMR, GPTMR_CH_RLD_STAT_MASK(UART_DIR_SWITCH_GPTMR_CHANNEL))) {
        ...
        for (uint8_t i = 0; i < pending; i++) {
            ...
            if (uart->port.port.options & SERIAL_BIDIR) {
                if (uart->tx.pin) {
                    HPM_IOC->PAD[IOCIndex(IOGetByTag(uart->tx.pin))].FUNC_CTL = 0;
                }
                if (uart->rx.pin) {
                    HPM_IOC->PAD[IOCIndex(IOGetByTag(uart->rx.pin))].FUNC_CTL = uart->rx.af;
                }
            }
...
void uartEnableTxInterrupt(uartPort_t *uartPort)
{
    ...
    // writing the first byte, then enabling the TXE interrupt for the rest.
    uartDevice_t *uart = container_of(uartPort, uartDevice_t, port);
    uint32_t irq = uart_get_enabled_irq((UART_Type *) (uartPort->USARTx));

    if (irq & uart_intr_tx_slot_avail) {
    ...
    uint32_t irqState = disable_global_irq(CSR_MSTATUS_MIE_MASK);
    uartQueueRemove(uart);
    enable_global_irq(irqState);

    if (uartPort->port.options & SERIAL_BIDIR) {
        if (uart->tx.pin) {
            HPM_IOC->PAD[IOCIndex(IOGetByTag(uart->tx.pin))].FUNC_CTL = uart->tx.af;
        }
        if (uart->rx.pin) {
            HPM_IOC->PAD[IOCIndex(IOGetByTag(uart->rx.pin))].FUNC_CTL = 0;
        }
    }
    uart_write_byte((UART_Type *) (uartPort->USARTx), uartPort->port.txBuffer[uartPort->port.txBufferTail]);
    uartPort->port.txBufferTail = (uartPort->port.txBufferTail + 1) % uartPort->port.txBufferSize;

    // Always request a TX-slot interrupt after the kick-start byte.  It is
    // also the completion event that schedules RX restoration for a
    // single-byte half-duplex transfer.
    uart_enable_irq((UART_Type *) (uartPort->USARTx), uart_intr_tx_slot_avail);
}
