...
void usbd_cdc_acm_bulk_in(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    if (txLen) {
        usbd_ep_start_write(busid, CDC_IN_EP, txBuffer, txLen);
    } else {
        epTxBusyFlag = false;
    }
}
...
{
    if (writeBufferRd == writeBufferWr) {
        return;
    }

    epTxBusyFlag = true;
    uint32_t len = 0;
    while (writeBufferRd != writeBufferWr && len < CDC_ACM_TX_BUFFER_SIZE) {
        txBuffer[len++] = writeBuffer[writeBufferRd];
        writeBufferRd = (writeBufferRd + 1) % CDC_ACM_TX_BUFFER_SIZE;
    }
    usbd_ep_start_write(cdcBusid, CDC_IN_EP, txBuffer, len);
}
